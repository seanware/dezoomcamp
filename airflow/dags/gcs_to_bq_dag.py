...
DATASET = "tripdata"
COLOR_RANGE = {'yellow': 'tpep_pickup_datetime', 'green': 'lpep_pickup_datetime'}
INPUT_PART = "raw"
...
with DAG(
    ...
    for color, ds_col in COLOR_RANGE.items():
        ...
        if color == 'green':
            CREATE_BQ_TBL_QUERY = (
            f"CREATE OR REPLACE TABLE {BIGQUERY_DATASET}.{color}_{DATASET} \
            PARTITION BY DATE({ds_col}) \
            AS \
            SELECT * EXCEPT(ehail_fee) FROM {BIGQUERY_DATASET}.{color}_{DATASET}_external_table;"
                )
        else:
            CREATE_BQ_TBL_QUERY = (
            f"CREATE OR REPLACE TABLE {BIGQUERY_DATASET}.{color}_{DATASET} \
